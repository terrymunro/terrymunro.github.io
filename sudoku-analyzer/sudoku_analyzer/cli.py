...
Usage examples (from the sudoku-analyzer directory):

    python -m sudoku_analyzer validate PUZZLE
    python -m sudoku_analyzer solve PUZZLE
    python -m sudoku_analyzer rate PUZZLE --json
    python -m sudoku_analyzer harden PUZZLE --greedy

PUZZLE is 81 characters (1-9 for givens, 0 or . for blanks), or a path to a
file containing a grid; whitespace and ASCII grid decorations are ignored.
"""
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if ok and args.min_grade:
        ...
        required = grades.index(args.min_grade)
        actual = grades.index(analysis.rating.grade)
        if actual < required:
            if not args.json:
                print(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="sudoku_analyzer",
        description=(
            "Validate, grade, annotate and harden sudoku puzzles. "
            "Uniqueness is proved with Google OR-Tools CP-SAT; difficulty "
            "and annotations come from a guess-free human-technique solver."
        ),
    )
