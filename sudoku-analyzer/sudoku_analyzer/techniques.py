...
Placement = Tuple[int, int]  # (cell, digit)
Elimination = Tuple[int, int]  # (cell, digit)
...
def find_locked_candidates_pointing(board: Board) -> Optional[Step]:
...
def find_locked_candidates_claiming(board: Board) -> Optional[Step]:
...
def _find_naked_subset(board: Board, size: int) -> Optional[Step]:
    for unit_index, unit in enumerate(UNITS):
        open_cells = [
...
def _find_hidden_subset(board: Board, size: int) -> Optional[Step]:
    for unit_index, unit in enumerate(UNITS):
        digit_spots = {
...
def find_naked_quad(board: Board) -> Optional[Step]:
...
def find_hidden_triple(board: Board) -> Optional[Step]:
    return _find_hidden_subset(board, 3)


def find_hidden_quad(board: Board) -> Optional[Step]:
...
def _find_fish(board: Board, size: int) -> Optional[Step]:
    for digit in DIGITS:
...
def find_swordfish(board: Board) -> Optional[Step]:
    return _find_fish(board, 3)


def find_jellyfish(board: Board) -> Optional[Step]:
...
def find_xy_wing(board: Board) -> Optional[Step]:
...
def find_xyz_wing(board: Board) -> Optional[Step]:
...
class Technique:
    name: str
    cost: float
    finder: Callable[[Board], Optional[Step]]
